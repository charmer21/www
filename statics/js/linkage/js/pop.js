...
function open_linkage(id,name,container,linkageid) {
	...
	{
		content += '<a href="javascript:;" onclick="get_child(\''+container[i][0]+'\',\''+container[i][1]+'\',\''+container[i][2]+'\')" title="'+container[i][1]+'">'+container[i][1]+'</a>';
	}
	content += '</div></div>';
	art.dialog({title:name,id:'edit_'+id,content:content,width:'420',height:'250',style:'none_icon ui_content_m'}, submit);
}
var level = 0;

function submit() {
	set_val(currNodename,currNodeid);
	$("input[name='info["+currReturnid+"]']").val(currNodeid);
	art.dialog({id:'edit_'+currReturnid}).close();
}

function get_child(nodeid,nodename,parentid) {
	currReturnid = returnid;
	currNodeid = nodeid;
	...
	$.getJSON(url+'&callback=?',function(data){
		if(data) {
			...
			$.each(data, function(i,data){ 
				...
				content += '<a href="javascript:;" onclick="get_child(\''+container[0]+'\',\''+container[1]+'\',\''+container[2]+'\')" title="'+container[1]+'">'+container[1]+'</a>';	
			})
