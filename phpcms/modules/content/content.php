...
class content extends admin {
	...
	public function init() {
		...
		if(isset($_GET['catid']) && $_GET['catid'] && $this->categorys[$_GET['catid']]['siteid']==$this->siteid) {
			...
			if($_SESSION['roleid'] > 7)
				$where .= " AND league = ".$_SESSION['roleid'];
	...
	//刷新时间算法    规则：1.更新时间至少比发布时间晚一天    2.发布时间距现在至少15天     3.更新时间为最近三天
	...
	public function inforelate() {
		...
		$modelid = $category['modelid'];
		$this->db->set_model($modelid);
		$tb = $this->db->table_name;
		$tbdata = $this->db->table_name.'_data';
		...
		if (isset($_POST['ids']) && !empty($_POST['ids'])) {
			$ids = $_POST['ids'];
			foreach ($ids as $id) {
				$this->db->table_name = $tbdata;
				$info = $this->db->get_one(array('id'=>$id));
				
				$relation = $info['relation'];
				$info = $this->db->get_one(array('id'=>$id));
				if($relation==null || $relation==''){
					$r = $this->relate($info, $rtype, $tb);
					if($r!=null && $r!=''){
						$this->db->table_name = $tbdata;
						$this->db->update(array('relation'=>$r),array('id'=>$id));
					}
				}
			}
			showmessage(L('update_success'),HTTP_REFERER);
			...
			$this->db->table_name = $tbdata;
			$infos = $this->db->select('', 'id, relation');
			foreach ($infos as $info) {
				$id = $info['id'];
				$relation = $info['relation'];
				if($relation==null || $relation==''){
	...
	private function relate($info, $rtype, $tb){
		$relation = '';
		$MAX_NUM = 35;
		$MIN_NUM = 20;
		...
		//根据区域关联
		$id = $info['id'];
		$this->db->table_name = $tb;
		$info = $this->db->get_one(array('id'=>$id));
		$zone = $info['zone'];
		$coal_type = $info[$rtype.'_type'];
		if($zone!=null&&$zone!=''){
			//优先：相同地区相同类型信息
			$where = "`zone`=$zone";
			if($rtype!='') 
				$where = $where." AND ".$rtype."_type=$coal_type";
			
			$infos = $this->db->select($where, "id", rand($MIN_NUM, $MAX_NUM), "RAND()");
			$len = count($infos);
			if($len > 0){
				foreach ($infos as $i) {
					$relation .= $i['id'].'|';
			...
			if($len < $MIN_NUM){
				$where2 = "`zone`=$zone";
				if($rtype!='') 
					$where2 = $where2. " OR ".$rtype."_type=$coal_type OR 1";
				else 
					$where2 = $where2. " OR 1";
				
				$infos2 = $this->db->select($where2, "id", rand($MIN_NUM-$len, $MAX_NUM-$len), "RAND()");
				$len2 = count($infos2);
				if($len2 > 0){
					foreach ($infos2 as $i) {
						$relation .= $i['id'].'|';
	...
	public function refreshzone() {
		...
		if (isset($_POST['ids']) && !empty($_POST['ids'])) {
			...
			foreach ($ids as $id) {
				$this->db->table_name = $tb;
				$id = $infodata['id'];
				$info = $this->db->get_one("id=$id", "zone");
				$ozone = $info['zone'];
				if($ozone!=$DEFAULTZONE && $ozone!=null && $ozone!='') continue;
				
				$zone = $this->trans_zone($info);
				if($zone!=null && $zone!=''){
					$this->db->table_name = $tb;
					$this->db->update(array('zone'=>$zone),array('id'=>$id));
				}
			}
			showmessage(L('update_success'),HTTP_REFERER);
	...
	private function trans_zone($info){
		$ip = $info['ip'];
		if($ip==null || $ip=='') return null;
		$arr = $this->get_zone_name($ip);
		...
		if($name=='') return null;
		$linkage = $this->linkage_db->get_one("`name` LIKE  '%$name%' AND keyid=1", "linkageid", "linkageid DESC");
var_dump($name);
var_dump($linkage['linkageid']);
echo '<hr>';
		return $linkage['linkageid'];
	}
	
	/**
	 * @category 通过ip获取地址  
	 * @return Array ( [ip] =>'' [pro] =>'' [city] =>'' [region] =>'' [addr] =>'' [regionNames] =>'' )
	 */
	private function get_zone_name($ip) {
		...
		preg_match('|IPCallBack\((.*?)\);} *|',$out,$matchs);
		$json = $matchs[1];
