...
class index {
	...
	//JSON 输出当前登录用户信息
	public function json_memberdetail() {
		$userid = $_REQUEST['u'];
		$field = $_REQUEST['f'];
		if($field=='password')
			echo json_encode('');
		$info = get_memberinfo($userid);
		unset($info['password']); //过滤密码
		$detail = get_memberdetail($userid);
		$member = array_merge($info,$detail);

		if($field)
			exit(json_encode($member[$field]));
...
//		exit();
		$tz = '';
		if(isset($data[zone])&&$data[zone]!=null&&count($data[zone])>0){
			foreach ($data[zone] as $arr) {
				$tz .= '_'.$arr['name'];
			}
		}
		...
		$SEO = seo($siteid, $catid, $title.$tz, $description, $seo_keywords);
