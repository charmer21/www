		...
		if ($html = self::get_html($url, $config)) {
			if ($config['sourcetype'] == 4) { //RSS
				...
				foreach ($out[1] as $k=>$v) {
					if (str_exists($v, 'href = ') && str_exists($v, '.shtml')){
						$v = str_replace('href = ', 'href="', $v);
						$v = str_replace('.shtml', '.shtml"', $v);
					}

					if (preg_match('/href=[\'"]?([^\'" ]*)[\'"]?/i', $v, $match_out) || preg_match('/href\s*=\s*(([^]*)|[^\s])\s*', $v, $match_out)) {
