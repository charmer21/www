...
class member_verify extends admin {
	...
	function manage() {
		...
		if($_SESSION['roleid'] == 1) {
			...
			$where = array('status'=>$status, 'league'=>$_SESSION['roleid']);
		}
	...
	function pass() {
		if (isset($_POST['userid'])) {
			...
			foreach($userarr as $v) {
				$status = $this->client->ps_mymember_register($v['username'], $v['password'], $v['email'], $v['league'], $v['regip'], $v['encrypt']);
				if ($status > 0) {
