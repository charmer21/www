...
class content extends foreground {
	...
	public function publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			//添加用户群组信息
			$info['league'] = $memberinfo['league'];
			//如果没有city参数时，添加默认地区
			if(!isset($_POST['city']))
				$info['city'] = getcity('default','linkageid');
			$this->content_db->siteid = $siteid;

			//附加关联信息
			$relation = $_POST['info']['relation'];
			if($relation==null || $relation==''){
				if($catid==28||$catid==29){
					$info['relation'] = $this->inforelate($table_name, $this->COAL);
				} else if($catid==32||$catid==143||$catid==144){
					$info['relation'] = $this->inforelate($table_name, $this->TRANSPORT);
				} else if($catid==145||$catid==146||$catid==147){
// 					$info['relation'] = $this->inforelate($table_name, $this->TRUCK);
					$info['relation'] = $this->inforelate($table_name);
				}
			}
	...
	public function info_publish() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$relation = $_POST['info']['relation'];
			if($relation==null || $relation==''){
				$info['relation'] = $this->inforelate($table_name, 'coal');
			}
...
//				$zone_arrchild = show_mylinkage($info_linkageid,$cityid,$cityid);
				$zone_arrchild = show_mylinkage($info_linkageid,$cityid, 0);//根节点parentId=0
				include template('member', 'info_content_publish_select');
				exit;
	...
	//插入相关信息属性
	private function inforelate($table_name, $rtype){
		$relation = '';
		$MAX_NUM = 35;
		$MIN_NUM = 20;
		
		//根据区域关联
		$zone = $_REQUEST['info']['zone'];
		if($rtype)
			$type = $_REQUEST['info'][$rtype.'_type'];
		if($zone!=null&&$zone!=''){
			$this->content_db->table_name = $table_name;
			//优先：相同地区相同类型信息
			$where = "zone=$zone";
			if($rtype!=undefined && $rtype!='') 
				$where = $where." AND ".$rtype."_type in ($type)";
			
			$infos = $this->content_db->select($where, "id", rand($MIN_NUM, $MAX_NUM), "RAND()");
			$len = count($infos);
			if($len > 0){
			...
			if($len < $MIN_NUM){
				$where2 = "zone=$zone";
				if($rtype!=undefined && $rtype!='') 
					$where2 = $where2. " OR ".$rtype."_type=$type OR 1";
				else 
					$where2 = $where2. " OR 1";
				
				$infos2 = $this->content_db->select($where2, "id", rand($MIN_NUM-$len, $MAX_NUM-$len), "RAND()");
				$len2 = count($infos2);
				if($len2 > 0){
