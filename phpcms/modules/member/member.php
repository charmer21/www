...
class member extends admin {
	...
	function search() {
		...
		if (isset($_GET['search'])) {
			...
			if($_SESSION['roleid'] == 1) {
				...
				$siteid = get_siteid();
				//查询默认判断当前管理员群组
				$where .= "`siteid` = '$siteid' AND `league` = ".$_SESSION['roleid']." AND ";
			}
	...
	function add() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			//附加群组的注册
			$status = $this->client->ps_mymember_register($info['username'], $info['password'], $info['email'], $info['league'], $info['regip']);
