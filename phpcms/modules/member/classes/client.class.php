...
// #############################################################	
	 ...
	 * @param string $password 	密码
	 * @param string $email		email
	 * @param string $league	联盟群组
	 * @param string $regip		注册ip
	 * @param string $random	密码随机数
	 * @return int {-1:用户名已经存在 ;-2:email已存在;-3:email格式错误;-4:用户名禁止注册;-5:邮箱禁止注册；int(uid):成功}
	 */
	public function ps_mymember_register($username, $password, $email, $league=0, $regip='', $random='') {
		if(!$this->_is_email($email)) {
			return -3;
		}
		 
		return $this->_ps_send('register', array('username'=>$username, 'password'=>$password, 'email'=>$email, 'league'=>$league, 'regip'=>$regip, 'random'=>$random));
	}
// #############################################################	
	/**
	 * 用户登陆
