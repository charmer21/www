 ...
 * 传入field，取用户$field字段信息
 */
function get_memberdetail($userid, $modelid=10) {
	if(!is_numeric($userid)) {
		return false;
	} else {
		static $memberdetail;
		if (!isset($memberdetail[$userid])) {
			$memberdetail_db = pc_base::load_model('member_model');
			$memberdetail_db->set_model($modelid);
			$memberdetail[$userid] = $memberdetail_db->get_one(array('userid'=>$userid));
		}
// 		if(!empty($field) && !empty($memberdetail[$userid][$field])) {
// 			return $memberdetail[$userid][$field];
// 		} else {
