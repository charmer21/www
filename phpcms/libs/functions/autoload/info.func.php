...
function structure_filters_sql($modelid,$cityid='') {
	...
	foreach ($_GET as $k=>$r) {
		if(in_array($k,$fields_key) && (intval($r)!=0 || floatval($r)!=0) && ($fields[$k]['filtertype'] || $fields[$k]['rangetype'])) {
 ...
 * @param  $showall 是否显示全部
 */
function show_mylinkage($keyid, $linkageid = 0, $toppatentid = 0, $modelid = '', $fieldname='zone' ,$showall = 1) {
 ...
 * @param  $leveltype 获取类型 parentid 获取父级id child 获取是否有子栏目 arrchildid 获取子栏目数组
 */
